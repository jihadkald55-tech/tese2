...
import { Search, X, UserPlus, Users, GraduationCap, Briefcase } from 'lucide-react'
...
export default function UserSearchModal({ isOpen, onClose, onSelectUser }: UserSearchModalProps) {
  ...
  const handleSearch = (query: string) => {
    ...
    // تصفية حسب نوع المستخدم الحالي
    const filteredUsers = allUsers.filter((u: User) => {
      // عدم إظهار المستخدم الحالي
      if (u.id === currentUser?.id) return false
      
      // للطالب: إظهار الأساتذة فقط
      if (currentUser?.role === 'student' && u.role !== 'professor') return false
      
      // للأستاذ: إظهار الطلاب فقط
      if (currentUser?.role === 'professor' && u.role !== 'student') return false
              ...
              <div className="bg-white dark:bg-dark-card rounded-[24px] shadow-google-lg dark:shadow-dark border border-medad-border dark:border-dark-border overflow-hidden flex flex-col max-h-full">
                ...
                <div className="p-6 border-b border-medad-border dark:border-dark-border flex-shrink-0">
                  <div className="flex items-center justify-between mb-4">
                    <h2 className="text-2xl font-bold text-medad-ink dark:text-dark-text">
                      {currentUser?.role === 'student' ? 'البحث عن مشرف' : 'البحث عن طالب'}
                    </h2>
                    <button
                      onClick={onClose}
                  ...
                  <div className="relative">
                    ...
                    <input
                      ...
                      placeholder={currentUser?.role === 'student' ? 'ابحث عن أستاذ...' : 'ابحث عن طالب...'}
                ...
                <div className="overflow-y-auto p-4 flex-1 min-h-0">
                  {searchQuery === '' ? (
                    ...
                    <div className="space-y-2">{searchResults.map((user) => (
                      <motion.button
                        key={user.id}
